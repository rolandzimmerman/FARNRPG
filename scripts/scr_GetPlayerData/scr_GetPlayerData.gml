...
function scr_GetPlayerData(_char_key) {
    ...
    if (!is_undefined(_persistent_stats_source)
        && (is_struct(_persistent_stats_source) || instance_exists(_persistent_stats_source))) {
        ...
        var _req_xp = (script_exists(scr_GetXPForLevel))
                      ? scr_GetXPForLevel(_battle_data.level + 1)
                      : 100;
        _battle_data.xp_require = variable_struct_exists(_persistent_stats_source, "xp_require")
                                 ? _persistent_stats_source.xp_require
                                 : _req_xp;
    } else {
        // Fallback full defaults
        ...
        _battle_data.xp_require = (script_exists(scr_GetXPForLevel))
                                  ? scr_GetXPForLevel(2)
                                  : 100;
    }
    ...
    // --- New: Add your status‐effect spells here ---
    var s = _battle_data.skills;


    show_debug_message(" [GetPlayerData] Created battle data for " 
        + _char_key 
        + ": HP " + string(_battle_data.hp) + "/" + string(_battle_data.maxhp) 
        + " | Level " + string(_battle_data.level) 
        + " | XP " + string(_battle_data.xp) + "/" + string(_battle_data.xp_require));
