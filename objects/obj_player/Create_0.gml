...
if (!variable_instance_exists(id, "persistent_data_initialized")) {
    ...
    if (_is_new_game) {
        ...
        if (ds_exists(global.party_current_stats, ds_type_map) && !ds_map_exists(global.party_current_stats, _hero_key)) {
            ...
            // --- <<< ADDED LOGGING Around Skill Clone >>> ---
            var _skills_from_source = []; // Default empty
             if (variable_struct_exists(_base_data, "skills") && is_array(_base_data.skills)) {
                 _skills_from_source = _base_data.skills; 
                 show_debug_message("    -> Found skills array REFERENCE in base_data. Length: " + string(array_length(_skills_from_source))); 
             } else { show_debug_message("    -> Skills array missing/invalid in base_data!"); }
            
             show_debug_message("    -> Attempting variable_clone on source skills array (Length: " + string(array_length(_skills_from_source)) + ")");
            var _skills_arr_persistent = variable_clone(_skills_from_source, true); // <<< TRY STANDARD CLONE AGAIN
             show_debug_message("    -> Cloned skills for persistent struct using variable_clone. Resulting Length: " + string(array_length(_skills_arr_persistent))); 
             // --- <<< END LOGGING >>> ---
            ...
            var _initial_hero_stats = {
                ...
                skills:     is_array(_skills_arr_persistent) ? _skills_arr_persistent : [], // Assign the cloned array safely
                ...
                class:      variable_struct_get(_base_data, "class") ?? "Adventurer", 
                character_key: _hero_key
                // NOTE: battle_sprite, attack_sprite, cast_sprite, item_sprite are NOT saved in persistent data
            };
            _initial_hero_stats.hp = _initial_hero_stats.maxhp; 
            _initial_hero_stats.mp = _initial_hero_stats.maxmp; 

             // --- <<< ADDED LOGGING: Check struct just before saving >>> ---
              show_debug_message("    -> Checking _initial_hero_stats struct BEFORE adding to map:");
              if (variable_struct_exists(_initial_hero_stats, "skills") && is_array(_initial_hero_stats.skills)) {
                   show_debug_message("        -> Skills Array Length in _initial_hero_stats: " + string(array_length(_initial_hero_stats.skills))); // <<< CHECK THIS!
              } else { show_debug_message("        -> Skills Array MISSING or invalid in _initial_hero_stats!"); }
             // --- <<< END LOGGING >>> ---
...
turnCounter = 0; 
attack_anim_speed = 0.5; // Default attack animation speed
idle_sprite = sprite_index; // Initialize with current sprite
attack_sprite_asset = -1;   
casting_sprite_asset = -1; 
item_sprite_asset = -1; 
sprite_before_attack = sprite_index; 
original_scale = image_xscale; // Initialize with current scale
