...
// -----------------------------------------------------------------------------
/// @description Draws the equipment slots, current gear, character stats, and item selection list.
...
if (!variable_instance_exists(id, "equipment_data") || !is_struct(equipment_data)) {
    var _ew = display_get_gui_width(); var _eh = display_get_gui_height();
    draw_set_halign(fa_center); draw_set_valign(fa_middle); draw_set_color(c_red);
...
var line_h = 24;
var col1_x = margin;
var col2_x = col1_x + 100;
var col3_x = gui_w / 2 + margin;
var col4_x = col3_x + 80;

// --- Calculate Character Stats (Base + Equipment) ---
var display_stats = scr_CalculateEquippedStats(equipment_data);
if (!is_struct(display_stats)) { draw_text(10, 10, "Stat Calculation Error!"); return; }
...
draw_set_halign(fa_center); draw_set_color(col_text);
...
for (var i = 0; i < slot_count; i++) {
    ...
    if (is_string(current_item_key)) {
        ...
        if (is_struct(item_info) && variable_struct_exists(item_info, "name")) { item_label = item_info.name; }
        else { item_label = "(invalid item!)"; }
    }
...
if (menu_state == EEquipMenuState.SelectingItem) {
    // Sub-menu positioning and dimensions
    ...
    var sub_width = sub_stat_diff_x + 80 - sub_x;
    ...
    var sub_height = (sub_visible_items + 1) * line_h;
    sub_height = min(sub_height, gui_h - sub_y - margin);
    if (sub_y + sub_height > gui_h - margin) { sub_y = gui_h - margin - sub_height; }
    sub_y = max(margin, sub_y);

    // Draw sub-menu box
    draw_set_color(c_navy); draw_set_alpha(0.9);
    draw_rectangle(sub_x, sub_y, sub_x + sub_width, sub_y + sub_height, false);
    draw_set_alpha(1.0); draw_set_color(c_white);
    draw_rectangle(sub_x, sub_y, sub_x + sub_width, sub_y + sub_height, true);
    ...
    for (var i = 0; i < sub_visible_items; i++) {
        ...
        var current_draw_y = item_list_draw_y + i * line_h;
        ...
        if (is_selected_item && variable_struct_exists(id, "item_submenu_stat_diffs") && is_struct(item_submenu_stat_diffs)) {
            ...
            // --- Draw HP Diff ---
            var val_hp = item_submenu_stat_diffs.hp_total ?? 0; if (val_hp != 0) { var s_hp = val_hp > 0 ? "+" : ""; var col_hp = val_hp > 0 ? col_stat_increase : col_stat_decrease; draw_set_color(col_hp); draw_text(diff_draw_x, diff_base_y + diff_line * diff_line_h, "HP " + s_hp + string(val_hp)); diff_line++; }
            // --- Draw MP Diff ---
            var val_mp = item_submenu_stat_diffs.mp_total ?? 0; if (val_mp != 0) { var s_mp = val_mp > 0 ? "+" : ""; var col_mp = val_mp > 0 ? col_stat_increase : col_stat_decrease; draw_set_color(col_mp); draw_text(diff_draw_x, diff_base_y + diff_line * diff_line_h, "MP " + s_mp + string(val_mp)); diff_line++; }
            // --- Draw ATK Diff ---
            var val_atk = item_submenu_stat_diffs.atk ?? 0; if (val_atk != 0) { var s_atk = val_atk > 0 ? "+" : ""; var col_atk = val_atk > 0 ? col_stat_increase : col_stat_decrease; draw_set_color(col_atk); draw_text(diff_draw_x, diff_base_y + diff_line * diff_line_h, "ATK " + s_atk + string(val_atk)); diff_line++; }
            // --- Draw DEF Diff ---
            var val_def = item_submenu_stat_diffs.def ?? 0; if (val_def != 0) { var s_def = val_def > 0 ? "+" : ""; var col_def = val_def > 0 ? col_stat_increase : col_stat_decrease; draw_set_color(col_def); draw_text(diff_draw_x, diff_base_y + diff_line * diff_line_h, "DEF " + s_def + string(val_def)); diff_line++; }
            // --- Draw MATK Diff ---
            var val_matk = item_submenu_stat_diffs.matk ?? 0; if (val_matk != 0) { var s_matk = val_matk > 0 ? "+" : ""; var col_matk = val_matk > 0 ? col_stat_increase : col_stat_decrease; draw_set_color(col_matk); draw_text(diff_draw_x, diff_base_y + diff_line * diff_line_h, "MATK " + s_matk + string(val_matk)); diff_line++; }
            // --- Draw MDEF Diff ---
            var val_mdef = item_submenu_stat_diffs.mdef ?? 0; if (val_mdef != 0) { var s_mdef = val_mdef > 0 ? "+" : ""; var col_mdef = val_mdef > 0 ? col_stat_increase : col_stat_decrease; draw_set_color(col_mdef); draw_text(diff_draw_x, diff_base_y + diff_line * diff_line_h, "MDEF " + s_mdef + string(val_mdef)); diff_line++; }
            // --- Draw SPD Diff ---
            var val_spd = item_submenu_stat_diffs.spd ?? 0; if (val_spd != 0) { var s_spd = val_spd > 0 ? "+" : ""; var col_spd = val_spd > 0 ? col_stat_increase : col_stat_decrease; draw_set_color(col_spd); draw_text(diff_draw_x, diff_base_y + diff_line * diff_line_h, "SPD " + s_spd + string(val_spd)); diff_line++; }
            // --- Draw LUK Diff ---
            var val_luk = item_submenu_stat_diffs.luk ?? 0; if (val_luk != 0) { var s_luk = val_luk > 0 ? "+" : ""; var col_luk = val_luk > 0 ? col_stat_increase : col_stat_decrease; draw_set_color(col_luk); draw_text(diff_draw_x, diff_base_y + diff_line * diff_line_h, "LUK " + s_luk + string(val_luk)); diff_line++; }
        }
    } // End for loop drawing items

     // Draw Scroll Indicators for Item List
     var indicator_x = sub_x + sub_width / 2;
     if (item_submenu_scroll_top > 0) { draw_set_halign(fa_center); draw_set_color(c_aqua); draw_text(indicator_x, sub_y - line_h * 0.1, "▲"); draw_set_halign(fa_left); }
     if (item_submenu_scroll_top + sub_visible_items < item_count) { draw_set_halign(fa_center); draw_set_color(c_aqua); draw_text(indicator_x, sub_y + sub_height - line_h * 0.6, "▼"); draw_set_halign(fa_left); }
} // End drawing item selection sub-menu

// --- Footer Controls Help Text ---
var footer_y = gui_h - margin - line_h;
draw_set_color(c_aqua); draw_set_halign(fa_left);
var help_text = "";
switch(menu_state) {
    case EEquipMenuState.BrowseSlots: help_text = "[↑/↓] Slot   [←/→] Char   [Confirm] Select Item   [Cancel] Back"; break;
    case EEquipMenuState.SelectingItem: help_text = "[↑/↓] Item   [Confirm] Equip Item   [Cancel] Back to Slots"; break;
}
draw_text(margin, footer_y, help_text);

// --- Reset Drawing Settings ---
draw_set_font(-1); draw_set_color(c_white); draw_set_halign(fa_left); draw_set_valign(fa_top); draw_set_alpha(1);